...
void 	GameView::renderBelote(Joueur you, Joueur beloteur, int bel) {
	if (bel==0) {
		return;
	}
	int player = (joueurToInt(beloteur)-joueurToInt(you)+4)%4;
	int x, y;
	if (player%2 == 0) {
	...
	SDL_RenderCopy(renderer, bel == 2 ? rebelote : belote, NULL, &rect);
}
...
void GameView::renderManche(Joueur you, Joueur who_deals, Paquet mypaquet, std::vector<int> taille_paquets, Enchere current_enchere, Joueur who_starts, Paquet pli_en_cours, int bel, Joueur beloteur) {
